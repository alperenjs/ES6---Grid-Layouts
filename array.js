...
}) //arrayimsi listeden array olştruyor
...
function sumAll(){
    ...
    const nums = Array.from(arguments); //argümentleri array yapyor reduce kullanabilmek için

    return nums.reduce((prev,next) => prev + next, 0);

}
sumAll(2,23,23,43,5,1,23,46);
...
// A R R A Y    F  I N  D ///////
// A R R A Y    F  I N  D ///////
//insta postları arasından code ile post çekiyor
const posts = [
    {
       "code":"BAcyDyQwcXX",
...
//      if(post.code === 'BAcJeJrQca9'){
//          return true;
//      }
//      return false;
//  })  aynısı kolayı aşağıda
...
// A R R A Y    S O ME //// A R R A Y    S O ME //// A R R A Y    S O ME //// A R R A Y    S O ME //


const ages1 = [32,15,19,22,12];

//is there at leaset one adult in group ?
const adultPresent = ages1.some(age => age >= 18);

//is everyone old enough to drink alch?
const allOldEnough = ages1.every(age=>age>=19)

// SPREAD OPERAATOR ... //// SPREAD OPERAATOR //// SPREAD OPERAATOR //// SPREAD OPERAATOR // 
//var olan iterationdaki elemanları başka bir iterationa ekliyor
//... spread operatördür ... concat yerine kullanılyor es6'da

const featured = ['Deep Dish', 'Peperon', 'Hawaiian'];
const specialty = ['Meatzza', 'Spicy', 'Margarita'];

const pizzas = [...featured, 'vegetarioan', ...specialty]

const fridayPizzas = [...pizzas];
fridayPizzas[0] = "Canadan pizza" //olarak değişrse ilk const pizza dğeişmemiş olyor


//spread exercise
const heading = $(".jump").text();

const spans = sparanWrap(heading);
